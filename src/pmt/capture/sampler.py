...
from pmt.capture.reader import read_text, read_bundle, load_json, read_manifest
from pmt.capture.writer import fsync_directory, atomic_write_bytes, atomic_write_json, append_manifest, write_compressed_bundle
from pmt.capture.discover import normalize_guid, telemetry_sort_key, validate_identifier, discover_inventory, inventory_identity
...
def machine_facts() -> Dict[str, Any]:
    ...
    uname = os.uname()
    firmware = {}
    for name in ("bios_vendor", "bios_version", "bios_date", "product_name", "board_name"):
        try:
            firmware[name] = read_text(Path("/sys/class/dmi/id") / name)
        except OSError:
            firmware[name] = None
    return {
...
def prepare_run(
    ...
    if requested_cpu is not None and requested_cpu not in os.sched_getaffinity(0):
        raise ValueError("--cpu must be a logical CPU allowed by the current cpuset")
    if run_path.exists():
        if not args.resume:
            raise RuntimeError(
        ...
        if requested_cpu is not None and requested_cpu != state.get("CollectorCPU"):
            raise RuntimeError("resume CPU does not match run.json")
    ...
    state = {
        ...
        "Endpoint": endpoint,
        "Status": "prepared",
        "CollectorSourceSHA256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "CollectorSourcesSHA256": {path.name: hashlib.sha256(path.read_bytes()).hexdigest()
                       for path in sorted(Path(__file__).parent.glob("*.py"))},
        "PlatformLabel": getattr(args, "platform", ""),
        "XMLVersionLabel": getattr(args, "xml_version", ""),
        "CreatedAt": utc_now(),
        "UpdatedAt": utc_now(),
        "SysfsRoot": str(args.sysfs_root.resolve()),
        "IntervalSeconds": args.interval_seconds,
        "CollectorCPU": requested_cpu,
        "RequestedSamples": args.samples,
        "ExpectedDurationSeconds": args.interval_seconds * args.samples,
        "PlannedSampleSpanSeconds": args.interval_seconds * (args.samples - 1),
...
def capture_locked(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        requested_cpu = state.get("CollectorCPU")
        if requested_cpu is not None:
            if requested_cpu not in original_affinity:
                raise ValueError("saved collector CPU is not allowed by the current cpuset")
            os.sched_setaffinity(0, {requested_cpu})
        state["EffectiveCPUs"] = sorted(os.sched_getaffinity(0))
        try:
...
def add_capture_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--cpu", type=int, help="run the collector on one allowed Linux logical CPU (preserved on resume)")
    parser.add_argument("--platform", default="", help="operator-supplied platform label, e.g. GNR; not hardware detection")
    parser.add_argument("--xml-version", default="", help="operator-supplied XML revision label; offline analysis records exact XML content")
