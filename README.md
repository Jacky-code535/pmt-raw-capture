...
当前源码为 **0.5.0-dev 开发预览版**。完整工具使用本仓库源码或本地构建的 `dist/pmt-raw-capture-0.5.0-dev.tar.gz`；GitHub 的 v0.4.3 Release 仍是旧采集包，没有新增分析命令。XML 注册表由使用者单独提供，不包含在本仓库或工具包中。

## 完整工具快速开始

在当前源码目录或解压后的 0.5.0-dev 工具目录执行：
...
下载后将压缩包放到被测主机。历史版本见 [Releases](https://github.com/Jacky-code535/pmt-raw-capture/releases)。
